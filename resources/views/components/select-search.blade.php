...
>
    ...
    <div
        ...
        x-bind:class="{'border-slate-300 ring ring-slate-200 ring-opacity-50':open, 'bg-gray-200 cursor-default':disabled}"
    ...
    >
        ...
        @isset($attributes['multiple'])
            <div class="flex flex-wrap space-x-1" x-cloak x-show="selected.length > 0">
                <template x-for="(key, index) in selected" :key="index">
                    <div class="text-gray-800 rounded-full truncate bg-slate-300 px-2 py-0.5 my-0.5 flex flex-row items-center">
        ...
        >
            ...
            <div class="relative z-30 w-full p-2 bg-white">
                <input x-model="search" x-on:click.prevent.stop="open=true" placeholder="Search" class="block w-full p-2 border border-gray-300 rounded-md focus:border-slate-300 focus:ring focus:ring-slate-200 focus:ring-opacity-50 sm:text-sm sm:leading-5">
            ...
            <div x-ref="dropdown" class="relative z-30 p-2 overflow-y-auto max-h-60" >
                ...
                <template x-for="(key, index) in Object.keys(options)" :key="index" >
                    @isset($attributes['multiple'])
                        <div
                            class="px-2 py-1"
                            x-bind:class="{'bg-gray-300 text-white hover:none':selected.includes(key), 'hover:bg-gray-100 cursor-pointer':!(selected.includes(key))}"
                    ...
                    @else
                        <div
                            class="px-2 py-1"
                            x-bind:class="{'bg-gray-300 text-white hover:none':selected==key, 'hover:bg-gray-100 cursor-pointer':!(selected==key)}"
                            @click.prevent.stop="selectOption(key)"
                            x-text="Object.values(options)[index]">
                        </div>
                    @endisset
